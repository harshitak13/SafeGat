"""
Graph topology builder for the 7x28 SUMO network.

Controlled junctions: 196 total across 10 irregular rows.
Edge index encodes bidirectional N<->S and E<->W adjacency between
all controlled nodes, based on their (row, col) grid positions.
"""
import torch
import numpy as np
...
from network.net_config import (
    ...
    TLS_NEIGHBOR_MAP, NUM_NODES,
)


def build_exact_edge_index() -> torch.Tensor:
    """
    Build COO edge_index for the 196-node irregular grid.
